...
if ( ! function_exists( 'ywca_admin_menu' ) ) {
	...
	function ywca_admin_menu() {
		$page_hook_suffix = add_submenu_page( 'tools.php', esc_html__( 'ACF Compare', 'surveys-plugin' ), esc_html__( 'ACF Compare', 'surveys-plugin' ), 'manage_options', 'ywca', 'ywca_admin_page' ); 
...
if ( ! function_exists( 'ywca_admin_page' ) ) {
	...
	function ywca_admin_page() {
		...
		$en_fields  = array();
		$dis_fields = array();
		$hide_empty = isset( $ywca_settings['hide-empty'] ) && $ywca_settings['hide-empty'] == 1 ? 'checked' : '';
		...
		<div class="wrap">
			...
			<form method="post" action="options.php" novalidate="novalidate" class="ywca-column-1">
				...
				<div class="ywca-field-sorter">
					...
					<div class="ywca-hide-empty">
						<label>
							<input type="checkbox" name="ywca_settings[hide-empty]" value="1" <?php echo esc_attr( $hide_empty ); ?>>
							Hide empty rows in compare table
						</label>
					</div>
...
if ( ! function_exists( 'ywca_yith_woocompare_filter_table_fields' ) ) {
	...
	function ywca_yith_woocompare_filter_table_fields( $fields, $products ) {
		// 
		$ywca_settings = get_option( 'ywca_settings', array() );
		$hide_empty = isset( $ywca_settings['hide-empty'] ) && $ywca_settings['hide-empty'] == 1;
		$hide_line = false;
		...
		if ( ! empty( $ywca_settings ) ) {
			...
			foreach ( $ywca_settings as $key => $field ) {
				if ( trim($key) != 'hide-empty' ) {
					$field_data = get_field_object( $field );
					if ( $hide_empty && count( $the_list ) ) {
						$counter = 0;
						foreach ($the_list as $product_id ) {
							if ( get_field( $field_data['key'], $product_id ) == '' ) {
								$counter++;
							}
						}
						$hide_line = $counter == count($the_list);
					}
